...
{
    // cả class này copy từ trên mạng không có hướng dẫn chi tiết
    public class Entity : DynamicObject, IXmlSerializable, IDictionary<string, object>
    {
        ...
        //21.2 mở rộng chỉnh sửa cho link xml vì mặc định nó sẽ hiển thị dạng <Links>System.Collections.Generic.List`1[Entites.Models.Link]<Links>
        private void WriteLinksToXml(string key, object value, XmlWriter writer)
        {
            writer.WriteStartElement(key);

            // kiểm tra có phải là danh sách
            if (value.GetType() == typeof(List<Link>))
            {
                foreach (var val in value as List<Link>)
                {
                    // lặp qua tất cả liên kết của value và đệ quy gán từng thuộc tính vào Href, Method,Rel
